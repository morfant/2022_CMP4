...
PerlinNoise::PerlinNoise() {
    ...
    mCalcFunc = make_calc_function<PerlinNoise, &PerlinNoise::next>();
    next(1);
}
...
double PerlinNoise::dot(const double* arr, double x, double y) {
    return arr[0] * x + arr[1] * y;
}

double PerlinNoise::easing(double t) {
    // 6t5-15t4+10t3
...
double PerlinNoise::CoherentNoiseGradient2D(double x, double y) {
...
double PerlinNoise::perlin2D(double x, double y, int octaves, double persistence) {
    double total = 0.0;
...
void PerlinNoise::next(int nSamples) {
    Unit* unit = (Unit*)this;
...
PluginLoad(PerlinNoiseUGens) {
    // Plugin magic
    ft = inTable;
    registerUnit<PerlinNoise::PerlinNoise>(ft, "PerlinNoise", false);
}
